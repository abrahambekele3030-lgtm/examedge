...
// ============================================================
// INDEXEDDB CACHE FOR OFFLINE QUESTIONS
// ============================================================

const IDB_DB_NAME = 'examedge_questions';
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = e => {
      ...
      if (!db.objectStoreNames.contains(IDB_STORE)) {
        db.createObjectStore(IDB_STORE, { keyPath: 'question_id' });
      }
    };
...
async function loadQuestionsFromIDB() {
  try {
...
async function saveQuestionsToIDB(questions) {
  try {
    const db = await openQuestionDB();
...
async function discoverAndLoadQuestions() {
  updateLoader(0, `Checking offline cache...`);

  // Try loading from IndexedDB first
  const cached = await loadQuestionsFromIDB();
  if (cached && cached.length > 0) {
    updateLoader(80, `Loaded ${cached.length.toLocaleString()} questions from offline cache...`);
    // Trigger background refresh from network (non-blocking)
    refreshQuestionsInBackground();
    return cached;
  }

  return await fetchQuestionsFromNetwork();
}

async function refreshQuestionsInBackground() {
  // Silently refresh from network without blocking the UI
  setTimeout(async () => {
    try {
      const questions = await fetchQuestionsFromNetwork(false);
      if (questions.length > 0) {
        await saveQuestionsToIDB(questions);
        console.log(`[BG] Cache refreshed: ${questions.length} questions`);
      }
    } catch (e) {
      console.warn('[BG] Background refresh failed:', e);
    }
  }, 5000);
}

async function fetchQuestionsFromNetwork(updateUI = true) {
  if (updateUI) updateLoader(0, `Scanning manifest...`);
  ...
  if (filePaths.length === 0) {
    if (updateUI) updateLoader(0, `Manifest failed. Fallback scanning...`);
    ...
    for (const result of results) {
      if (result && result.data && Array.isArray(result.data.questions) && result.data.questions.length > 0) {
        ...
        for (const q of data.questions) {
          // RELAXED VALIDATION: Only skip if there's no question text AND no answer options at all
          if (!q.question && !q.options) { skippedCount++; continue; }
          
          // Auto-generate missing question_id (prevents skipping valid questions)
          if (!q.question_id || String(q.question_id).trim() === '') {
            autoIdCounter++;
            q.question_id = `AUTO_${subjectName}_${gradeName.replace(/\s/g,'')}_${autoIdCounter}`;
          }
          
          // Auto-generate missing correct_answer from first available option if missing
          if (!q.correct_answer) {
            if (q.options && typeof q.options === 'object') {
              // Try to find answer from common alternative fields
              const altAnswer = q.answer || q.Answer || q.ANSWER;
              if (altAnswer) {
                q.correct_answer = String(altAnswer).toUpperCase().trim();
              } else {
                // Use first option key as fallback (visible as unanswered)
                const keys = Object.keys(q.options).filter(k => k.match(/^[A-Da-d]$/));
                if (keys.length > 0) q.correct_answer = keys[0].toUpperCase();
                else { skippedCount++; continue; }
              }
            } else { skippedCount++; continue; }
          }

          // Normalize options keys to uppercase (handle case-collision in JSON)
          if (q.options && typeof q.options === 'object') {
            const normalized = {};
            for (const [k, v] of Object.entries(q.options)) {
              const uk = k.toUpperCase();
              if (!normalized[uk]) normalized[uk] = v; // First one wins
            }
            q.options = normalized;
          }

          // Ensure correct_answer is uppercase
          if (q.correct_answer) q.correct_answer = String(q.correct_answer).toUpperCase().trim();
  ...
  if (skippedCount > 0) {
    console.warn(`[Data] Skipped ${skippedCount} questions with invalid/missing data`);
  }
  if (autoIdCounter > 0) {
    console.log(`[Data] Auto-generated IDs for ${autoIdCounter} questions`);
  }
...
function renderStats() {
  ...
  // Typeset all LaTeX in the stats view
  const statsView = document.getElementById('view-stats');
  if (statsView) setTimeout(() => typesetAll(statsView), 200);
}
...
function setupEventListeners() {
  ...
  // Mobile bottom nav — Menu button only (Practice/Exam/Search removed per requirements)

  // Study Timer — now draggable (see makeDraggable)
  initDraggableTimer();
  ...
  // Refresh Formulas button removed
...
// Math Re-render removed per requirements (Refresh Formulas feature removed)

// ============================================================
// DRAGGABLE STUDY TIMER (Pomodoro)
// ============================================================
let studyTimerInterval = null;
let studyTimeRemaining = 25 * 60; // 25 mins
let isStudyBreak = false;
let isStudyTimerRunning = false;
let _timerDragState = null;
let _timerLongPressTimer = null;
...
function resetStudyTimer() {
  ...
  if (timerBtn) {
    timerBtn.classList.remove('break', 'timer-running');
    timerBtn.style.opacity = '1';
  }
  showToast('⏱️ Timer reset to 25:00', 'info');
}
...
function toggleStudyTimer() {
  ...
  if (isStudyTimerRunning) {
    clearInterval(studyTimerInterval);
    isStudyTimerRunning = false;
    if (timerBtn) { timerBtn.style.opacity = '0.7'; timerBtn.classList.remove('timer-running'); }
  } else {
    isStudyTimerRunning = true;
    if (timerBtn) { timerBtn.style.opacity = '1'; timerBtn.classList.add('timer-running'); }
    ...
    studyTimerInterval = setInterval(() => {
      ...
      if (studyTimeRemaining <= 0) {
        ...
        studyTimeRemaining = isStudyBreak ? 5 * 60 : 25 * 60;
...
function initDraggableTimer() {
  ...
  const saved = localStorage.getItem('timer_pos');
  if (saved) {
    try {
  ...
  // Add reset button to timer
  const resetBtn = document.createElement('button');
  resetBtn.className = 'timer-reset-btn';
  resetBtn.title = 'Reset timer';
  resetBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.57"/></svg>`;
  resetBtn.addEventListener('click', e => { e.stopPropagation(); resetStudyTimer(); });
  ...
  timer.addEventListener('mousedown', e => {
    ...
    origLeft = rect.left;
    origTop = rect.top;
    timer.style.left = origLeft + 'px';
    timer.style.top = origTop + 'px';
    timer.style.right = 'auto';
    timer.style.bottom = 'auto';
  ...
  document.addEventListener('mousemove', e => {
    ...
    const newLeft = Math.max(0, Math.min(window.innerWidth - timer.offsetWidth, origLeft + dx));
    const newTop = Math.max(0, Math.min(window.innerHeight - timer.offsetHeight, origTop + dy));
  ...
  document.addEventListener('mouseup', () => {
    ...
    timer.style.cursor = '';
    if (hasDragged) {
      localStorage.setItem('timer_pos', JSON.stringify({ x: parseInt(timer.style.left), y: parseInt(timer.style.top) }));
    }
    // Re-enable animation after a tick
    setTimeout(() => {
      if (!isStudyTimerRunning) timer.style.animation = '';
...
async function init() {
  ...
  try {
    ...
    // Save to IndexedDB for offline use (async, non-blocking)
    saveQuestionsToIDB(questions).then(ok => {
      if (ok) console.log(`[IDB] ${questions.length} questions cached for offline use`);
    });

    // Tell service worker to pre-cache all data files
    ...
    const fromCache = questions[0]?._cached;
    const sourceLabel = fromCache ? '(from cache)' : '';
    console.log(`✅ ExamEdge loaded: ${questions.length.toLocaleString()} questions from ${State.subjects.size} subjects ${sourceLabel}`);
